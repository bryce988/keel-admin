...
- **超级管理员不走上面这套**：`sys_users.is_super = 1`，或**持有超管角色**
  （`sys_roles.code = 'ROLE-0001'`），两者都由 `PermissionService` 短路成 `['*']`。
  超管角色因此**没有可配置的授权集合**——界面上不显示授权按钮，接口层无论传什么
  都恒等重置回全部权限与全部数据范围。这样新增权限点对超管立即生效，
  不必重跑种子脚本，也不会出现「加了新菜单超管反而看不到」

  ⚠️ 两个来源意味着两处保护缺一不可（详见 `docs/database.md` §3.1）：
  持有超管角色的账号同样不可被操作；而**只有超管才能把超管角色授予别人**——
  否则任何持有 `sys:user:grantRole` 的人都能一步把自己变成超管
