...
- ⚠️ **「超管」有两个来源**：上面这个字段，以及**持有超级管理员角色**（`sys_roles.code = 'ROLE-0001'`）。
  后者由 `PermissionService::queryCodes()` 短路成 `['*']`，与 `is_super` 等效；
  数据范围也自动一致——该角色的 `data_scope = 1`，而 `DataScope::level()` 取名下角色的最宽值。
  这样做是为了让超管**不必维护一张授权表**：新增权限点立即生效，不依赖种子脚本重跑。

  两个来源意味着两处保护都要覆盖，漏一个就是绕过：
  - `UserService::findEditable()` 同时挡 `is_super` 与「持有超管角色」的账号，
    否则由角色获得超管身份的账号能被普通系统管理员重置密码或停用
  - `RoleService::assertAssignable()` 只允许超管把超管角色授予别人。
    这是新增来源带来的**提权路径**：`is_super` 界面点不出来，而角色可以授予，
    任何持有 `sys:user:grantRole` 的人本可以一步把自己变成超管
