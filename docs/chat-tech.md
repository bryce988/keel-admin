...
**会话对象里的 `avatar_members`**：群**没设头像**时才有，最多 4 位成员的 `{real_name, avatar}`，
群主排第一、其余按入群先后，前端拼成宫格头像（1 人占满 / 2 人左右 / 3 人上一下二 / 4 人 2×2）。
设了自定义头像或单聊时没有这个字段。会话列表里所有群一条 SQL 取完（`ROW_NUMBER() OVER (PARTITION BY conv_id)`），
不按群逐个查。

**群头像只收本系统上传的图**：`PUT …/group` 的 `avatar` 必须以 `/uploads/chat/` 开头且不含 `..`，
否则 400 + `21209`；传空串表示恢复成拼接头像；不传（`null`）表示不改。改名、改头像都会在群里落一条系统消息。
