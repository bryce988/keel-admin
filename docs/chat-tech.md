...
> 版本 v0.2 · 2026-09-20 · 目标里程碑 **M5**
> **进度：① 地基 · ② 一条消息跑通 · ③ 会话列表与未读 · ③.5 通讯录 已完成；④⑤⑥ 未开始。**
> 现在只能发纯文本单聊——图片文件发不了、发错撤不回、不能建群。逐项状态见范围表与验收清单。
...
| **④ 富消息与撤回** | 图片、文件、撤回、**已读标记渲染**、**发送失败点击重发** | 手机拍张照发出去，电脑能预览；两分钟内撤回，两端同时变灰 | 2~2.5 天 |
| **⑤ 群聊** | 建群、成员管理、@、群主撤回 | 三方群：手机 + 两个浏览器，验收清单「群」那组 | 2~3 天 |
| **⑥ 打磨与压测** | 虚拟滚动、断线重连、桌面通知、暗色模式、**刷新后保持滚动位置**、2000 连接 30 分钟 | 拔网线、切 wifi、挂起手机三种断线场景 + 内存曲线 | 2 天 |
