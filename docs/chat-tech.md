...
> 版本 v0.4 · 2026-09-21 · 目标里程碑 **M5**
> **进度：①②③ · ③.5 通讯录 · ④ 富消息与撤回 · ⑤ 群聊 已完成；⑥ 打磨压测 未开始。**
> 单聊与群聊都能用了。**缺的是打磨**：@ 的输入面板、桌面通知、虚拟滚动、
> 刷新后保持滚动位置，以及全部压测项。逐项状态见范围表与验收清单。
...
| **⑥ 打磨与压测** | 虚拟滚动、断线重连、桌面通知、暗色模式、**刷新后保持滚动位置**、**@ 输入面板**、2000 连接 30 分钟 | 拔网线、切 wifi、挂起手机三种断线场景 + 内存曲线 | 2 天 |
