...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/admin/upload` | 通用文件上传，返回 `{url, name, size, ext}`。**一步到位**，不做临时目录转正，见 §12.4 |
...
→ 201 Created
{
  "url":  "/uploads/chat/202609/3f7a9c21d84b6e05.png",
  "name": "评审纪要.png",     // 原始文件名，仅用于展示
  "size": 204813,
  "ext":  "png"
}
```

登录即可，**不挂权限点**。上传本身不是敏感动作，真正的边界在「传上来的文件被用在哪」——
换头像只能改自己、往公告里塞图要有公告的权限点。挂一个 `sys:upload` 只会变成
人人都授予的空权限点，既不增加安全性，又多一个会忘记配进权限树的东西
（漏配的表现是永久 403，只有用户点到才会发现，见 §7 的权限点约定）。

| 项 | 值 |
|---|---|
| 字段名 | `file`，与头像上传、用户导入保持一致 |
| `biz` | 业务标识，决定落盘目录。白名单 `common` / `chat` / `notice`，**写死在代码里**；传了白名单外的值按 `common` 处理，不报错 |
| 扩展名 | 图片 `jpg jpeg png gif webp`、文档 `pdf doc docx xls xlsx ppt pptx txt csv`、压缩包 `zip rar 7z`。白名单写死，不做成参数 |
| 大小上限 | 系统参数 `sys.upload.maxSize`，默认 20MB |
| 真实类型 | 图片类扩展名必须通过 `getimagesize()` 二次确认 |
| 频率 | 每人 60 次 / 分钟，超出 `429` + `Retry-After` |

**为什么不做「临时目录 + 转正」两段式**（这一条改了此前的设计）：

两段式要求每个调用方在业务保存成功后记得再调一次「转正」。漏一个，文件就会被临时目录的
清理任务删掉，而业务记录还指着那个地址——表现是「过几天图片打不开了」，
且只在清理周期之后才暴露，排查时很难联想到是上传漏了一步。

一步到位的代价是**用户选了文件却没提交**时留下孤儿文件，成本是磁盘；
两段式漏转正的代价是**数据损坏**。两害相权，选磁盘。

孤儿文件由**各业务自己的留存清理**带走：哪个业务用了这里返回的地址，
哪个业务就在自己的清理任务里把过期记录和对应文件一起删掉。
不为上传单独建清理机制——它不知道文件被谁用着，只能靠「多久没人引用」猜，猜错就是删了在用的文件。

**返回相对路径不带域名**——换域名、上 CDN 时库里的数据不用动（与头像一致）。

⚠️ `name` 是原始文件名，**只用于展示，绝不参与落盘路径**。落盘用随机名，
原始名里的 `../`、控制字符、超长串都进不了文件系统。
前端渲染它时要当普通文本转义，不要塞进 `innerHTML`。

⚠️ **`svg` 与 `html` 不在白名单里**，这是有意的：两者都能内嵌脚本，
而 `/uploads/` 是同域直出的，浏览器打开就是一次 XSS。要支持矢量图得先解决直出问题
（下载而非内联、或独立域名），不是往白名单里加一行的事。
