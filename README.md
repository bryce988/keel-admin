...
登录鉴权、完整 RBAC、系统管理、日志审计、异步任务、即时通讯与员工移动端均已落地。
...
能运行异步任务、能在同事之间实时聊天的管理后台，开发者可以直接增加业务模块。

框架已经包含五端入口、八个系统管理模块、五种通用页型模板、内部即时通讯和员工移动端 App。
...
| 运行能力 | webman 常驻内存，队列消费者、定时任务与聊天长连接网关随服务一起运行 |
...
| 聊天长连接 | ws://localhost:8788 | 开发环境前端直连；生产经 nginx 的 `/ws` 转发，不单独开端口 |
...
| 即时通讯 | 单聊与群聊、图片与文件、表情、@ 提醒、撤回、已读、置顶与免打扰、历史翻页、桌面通知与提示音；离职员工的会话只读 |
| 通讯录 | 组织架构只读视图，全员可找人（手机号、邮箱仍受字段权限约束），同事名片一键发起单聊 |
| 个人中心 | 资料维护、修改密码、换绑手机、个人登录记录 |
| 开发模板 | 标准列表、树表联动、主从、表单、详情五种页型，仅在开发环境注册 |
| 员工移动端 | 同账号体系登录；底部 `消息 / 通讯录 / 工作台 / 我的`，聊天与电脑端实时互通，公告收在工作台 |
...
| 服务端 | PHP 8.4+、webman 2.x、Eloquent、Redis（缓存、限流、队列与聊天消息扇出） |
| 数据存储 | MySQL 8.0+ |
...
```text
发消息  客户端 ──HTTP──→ webman ──落库（会话内递增序号）──→ Redis 发布
收消息  客户端 ←──WebSocket── 聊天网关进程 ←──订阅── Redis
```

**上行走 HTTP、长连接只负责下行**：发消息复用现有的鉴权、权限点、限流与日志，不必在长连接里再实现一遍。
每个会话的消息有连续序号，客户端发现断号或断线重连后按序号补拉，可靠性由数据库保证而不是长连接。
管理后台与员工端调用同一份服务端逻辑，手机发、电脑收天然一致。设计细节见
[docs/chat-tech.md](docs/chat-tech.md)。
...
│       ├── common/      多端共享能力
│       ├── middleware/  静态资源中间件
│       ├── process/     HTTP、定时任务与聊天网关（WebSocket）进程
│       └── queue/       异步任务消费者
├── docs/                跨项目共用的文档：接口契约、数据库设计、聊天设计、实施记录
├── docker/              开发与生产容器配置
└── PROJECT.md           完整项目文档
```
...
| [docs/chat-prd.md](docs/chat-prd.md) | 即时通讯产品文档：功能范围、交互规则与验收清单 |
| [docs/chat-tech.md](docs/chat-tech.md) | 即时通讯技术文档：消息可靠性、接口、长连接协议与部署 |
...
聊天长连接也从这个端口的 `/ws` 转发到网关进程，MySQL、Redis 与网关都不暴露宿主机端口。
部署前请更换数据库密码和 JWT 密钥。网络受限时可在 `.env` 中设置 `APK_MIRROR` 与 `COMPOSER_MIRROR`。

建议给站点配上 HTTPS：浏览器的桌面通知只在 HTTPS 下可用，用 `http://IP` 访问时聊天照常，
但「桌面通知」开关打不开（提示音不受影响）。

原生部署时，使用 systemd 或 supervisor 守护 `php start.php start -d`，并参考
`docker/nginx/default.conf` 配置端前缀转发、XFF、`/ws` 长连接转发、`/internal/` 拒绝和 `/uploads/` 静态资源规则。
...
| M4 联调加固 | ✅ | 57 项验收断言、30 分钟压测、断线重连与部署验证 |
| M5 员工移动端 | ✅ | 登录、工作台、公告消息、个人资料、跨域与续期机制 |
| 即时通讯 | ✅ | 后台与移动端同步交付：单聊群聊、附件、@、撤回、已读、通讯录与名片；长连接压测暂缓 |
