...
final class Guard
{
     ...
     * 写入的部门必须在自己的数据范围内
     *
     * 读侧有全局 Scope 自动注入，写侧没有（原因见 PROJECT.md §6.4），
     * 所以 dept_id 来自请求体的地方都要显式调它。拦的不是「改了别人的数据」，
     * 是把记录写到自己看不见的部门去，建完就找不回来，只能找超管收拾。
     *
     * dept_id = 0 只有「全部数据」能写，它不在任何受限集合里。
     * 不存在的部门与范围外的部门抛同一个错，顺手堵掉试探部门 id。
     *
