 ...
 * 「岗位下有用户」要提示去改人员归属，「字典编码重复」要把红框标在编码输入框上。
...
final class BizCode
{
    ...
    public const EXPORT_FILE_GONE = 20702;       // 导出文件已过期或被回收（任务记录仍在）

    // ---------------------------------------------------------------- 管理后台 · 岗位（208xx）
    // 排在导入导出后面而不是紧跟部门段：岗位原先一直借用部门的 20201/20203，
    // 补段时只能往后追加——已发出去的码不重排（见类注释）
    // 20801 已退役：岗位编码改由程序按主键生成，不存在「重复」这回事了。
